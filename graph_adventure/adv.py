...
while len(traversalPath) < 6000:
    ...
    if unexplored_exits:
        ...
        # initialise exits in current room
        ...
        traversalGraph[player.currentRoom.id][reverse_directions[random_exit]] = previous_room_id
    else:
        path_creation = True
        path = []
        ...
        while path_creation:
            ...
            popped = new_list.pop()
            player.travel(reverse_directions[popped])
            # add move to path
            path.append(reverse_directions[popped])
            # check if current_room_exits has not been explored
            current_room_exits = traversalGraph[player.currentRoom.id]
            unexplored_exits = []
            # add possible directions to unepxlored exits
...
while stack:
    ...
    if room.id not in traversalGraph:
        # create node/roomid in traversal graph
